...
def calcConformation(delta_r,vtMatrix,nma_index,mode_range):
    ...
    return (overlap_list, output, OverlapC, correlationC)  
...
def main(args):
   ...
   #Print warnings to user
    ...
    conformation = calcConformation(delta_r,vtMatrix,nma_index,mode_range)
    overlap_list = conformation[0]
...
#Calculating overlap/correlation per asymmetric unit, with chain breakdown
 ...
 #getIndexesPerChainPerUnit
    ...
    for i in range(number_of_protomersN):
        indexStart = 0
        IndexesPerChain = {}
        for k in commonK:
            s = indexStart+(i*totalC)
            e = indexStart+len(common_residues[k])+(i*totalC)
            IndexesPerChain[k]=[s,e]
            indexStart = e-(i*totalC)
        IndexesPerChainPerUnit[i+1]= IndexesPerChain
    ...
    for i in range(1,number_of_protomersN+1):
        ...
        for k in commonK:
            ...
            SpecificStart = SpecificIndexes[0]
            SpecificEnd = SpecificIndexes[1]
            deltaRS = delta_r[SpecificStart*3:SpecificEnd*3]
            nma_indexS = nma_index[SpecificStart:SpecificEnd]
            conformation = calcConformation(deltaRS,vtMatrix,nma_indexS,mode_range)

            overlap_list = conformation[0]
            output = conformation[1]
            OverlapC = conformation[2]
            correlationC = conformation[3]
            outputPerChainPerUnit.append("CHAIN: "+str(k)+"\n")

            outputPerChainPerUnit.append('MODE           Overlap              Correlation\n\n')
            for out in overlap_list:
                for o in output[out]:
